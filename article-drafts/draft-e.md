...
You're probably in the top five percent of security awareness in your peer group. You know how SIM swapping works. You've thought carefully about which devices touch which systems and why. If you were hit at some point — and a lot of people in this industry were — you've since locked down your carrier account and moved away from SMS-based 2FA. You know how these attacks work.

That used to be enough to make you a hard target. It isn't anymore — and the reason why matters for how you think about protecting your firm.

## The threat environment has changed

The social engineering attacks landing on crypto funds right now are not the phishing emails of two years ago. Those had tells: awkward language, suspicious urgency, something that felt slightly off. The baseline assumption that a security-aware person would catch them in the moment was reasonable.

That assumption is no longer safe.

Messages are now constructed from scraped LinkedIn data, public filings, and blockchain analytics, then refined by AI to match the tone and communication style of people you actually work with. Business email compromise attacks don't announce themselves — an attacker sits quietly in a compromised inbox for weeks, reads the thread history, maps the org chart, learns who approves what and how they write when they do it. When they move, the message fits your world well enough that catching it requires exactly the kind of sustained, skeptical attention that's hardest to maintain in the middle of a normal workday.
...
The point isn't that you'll be fooled. The point is that the attacks are now sophisticated enough that "we're savvy enough to spot it" is no longer a security posture. It's a bet. And the infrastructure-level question — what happens when the bet doesn't pay off — is the one most firms haven't answered.

## What personal OPSEC was built to do

Hardware keys, strong passwords, a VPN, carrier-hardened 2FA — these are all genuinely valuable. They raise the cost of certain attacks and close real vulnerabilities. The problem isn't that they don't work. The problem is what they were designed for: protecting an individual operating largely on their own.
...
## What those layers actually are

This is the part most security-aware fund operators haven't had occasion to think through, because it requires having run an actual security program rather than made a series of individual security decisions.
...
**Identity infrastructure.** A real identity provider, with every SaaS system gated through SSO, means access is granted and revoked in one place. When someone leaves, you disable their account in the IDP and they're out of everything — exchanges, cloud infrastructure, internal tools, all of it — rather than having to remember which systems they had access to and chase each one manually. Onboarding works the same way in reverse. Without this, the credential surface of a firm that's been operating for a few years is almost certainly larger than anyone can accurately account for.

**Credential surface audit.** Even with good identity infrastructure going forward, most firms have years of accumulated access that predates it. Former employees with live API keys to exchanges. Contractors whose admin rights outlasted their projects. Shared accounts with passwords multiple people know and nobody owns. Getting a current, accurate picture of who has access to what — and then closing the gaps — is unglamorous work. It's also where a significant share of low-effort attacks find their entry point.
...
None of these layers are exotic. None of them are particularly expensive relative to what they protect. The reason most funds don't have them isn't that they haven't heard of the concepts. It's that building and maintaining them requires someone to own the whole picture, and in a lean fund, that's nobody's job.

Security decisions get made in response to moments of concern. Something happens in the industry, or someone reads about a new attack, and a decision gets made. Each decision is reasonable. Nobody reviews whether it still holds six months later. Nobody notices what's missing from the overall posture. Nobody's job it is to evolve the program as the threat does.
...
Closing it doesn't require a full-time hire. It requires ten to twenty hours a month from someone senior enough to see the whole picture and accountable enough to keep it current — someone who reviews access on a regular cadence, responds when something flags, and updates the program as the threat environment changes.

The question isn't whether you've done the personal work. You probably have. The question is whether your firm has the structural layers that protect you when the attacks stop being something you can catch by paying attention.
